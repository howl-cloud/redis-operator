...
var _ = Describe("Primary isolation runtime guard", Label("network", "isolation"), func() {
	It("forces primary restart and converges without split-brain", func() {
		...
		defer cancel()
		const isolationTransitionTimeout = 12 * time.Minute
		...
		apiServerIPs, err := getKubernetesAPITargetIPs(ctx)
		Expect(err).NotTo(HaveOccurred())
		peerIPs, err := getPeerIPsExcluding(ctx, primaryPod.Name)
		Expect(err).NotTo(HaveOccurred())
		Expect(peerIPs).NotTo(BeEmpty())
		Expect(warmPrimaryIsolationPeerCache(ctx, testNamespace, primaryPod.Name)).To(Succeed())
		...
		_, err = faults.WaitForPrimaryChange(ctx, k8sClient, testNamespace, clusterName, primaryPod.Name, isolationTransitionTimeout)
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(faults.WaitForPhase(ctx, k8sClient, testNamespace, clusterName, "Degraded", 3*time.Minute)).To(Succeed())
		...
		recoveredPrimary, err := getPrimaryPod(ctx)
		Expect(err).NotTo(HaveOccurred())
		offsetAfter, err := faults.ReplicationOffset(ctx, testNamespace, recoveredPrimary.Name, redisPassword)
		Expect(err).NotTo(HaveOccurred())
