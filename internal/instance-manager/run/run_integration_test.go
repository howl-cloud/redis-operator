...
func TestWriteRedisConf_StartsServer(t *testing.T) {
	...
	// appendonly is overridden to "no" so Redis doesn't create appendonlydir inside
	// the bind-mounted host temp dir. Files created by the container's Redis user
	// (uid 999) cannot be chmod'd or removed by the test runner on Linux, which
	// causes t.TempDir's RemoveAll to fail. The unit tests already verify that
	// "appendonly yes" is present in the generated config file.
