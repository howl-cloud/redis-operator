...
func TestReconcileSecrets_AutoGeneratedAuthSecret_PersistsSpecReference(t *testing.T) {
	cluster := newTestCluster("test", "default", 1)
	...
	require.NotNil(t, updated.Spec.AuthSecret)
	assert.Equal(t, "test-auth", updated.Spec.AuthSecret.Name)
	assert.Contains(t, updated.Status.SecretsResourceVersion, "test-auth")

	var authSecret corev1.Secret
	err = c.Get(ctx, types.NamespacedName{Name: "test-auth", Namespace: "default"}, &authSecret)
	require.NoError(t, err)
	assert.NotEmpty(t, authSecret.Data["password"])
}
